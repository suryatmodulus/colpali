...
class CustomCollator:
    def __init__(
        ...
        if self.processor is not None:
            if self.processor.__class__.__name__ == "PaliGemmaProcessor" or self.processor.__class__.__name__ == "Idefics2Processor":
    ...
    def __call__(self, examples):
        ...
        if self.processor.__class__.__name__ == "Florence2Processor":
            return self.forward_vision_florence2(examples)
    ...
    def forward_vision_florence2(self, examples):
        texts_doc = []
        texts_query = []
        images = []
        for example in examples:
            ...
            image = example["image"].convert("RGB")
            images.append(image)
            texts_doc.append("<DocVQA>")   # '<DocVQA>' for florence2 finetuned

            if example["query"] is None:
                texts_query.append(None)
            else:
                query = f"<DocVQA>{example['query']}<pad><pad><pad><pad><pad>"
                texts_query.append(query)
